...
- 原始项目：BrainDance
- 备份内容：
  - 数据库：full_backup.dump.gz（包含存储元数据）
  - 配置文件：config/
  - 存储元数据：storage.objects, storage.buckets 等表
...
# 恢复备份的配置
cp config/seed.sql .
cp config/migrations/ migrations/
```
...
# 恢复数据库
docker exec -i supabase_db_BrainDance psql -U postgres -d postgres < database/full_backup.dump
...
注意：实际文件需要手动迁移，详见 STORAGE_MANUAL.md
