...
【停止条件 — 必须显式调用 stop_search】
任何形式的"我已经回答完了"都必须通过 stop_search 工具显式收口，绝对不要直接输出自然语言回答而不调用任何工具。
判断标准：
- 已有高置信度候选（如语义相似度 > 0.7 且有交叉证据）→ stop_search confidence ≥ 0.7
- 继续搜索只会重复已有信息，不会带来增量 → stop_search 并说明
- 问题本身不需要工具调用即可回答（闲聊、身份说明）→ 直接回答前也要先调 stop_search confidence=1
- 已拿到有效的预览/执行/对比/摘要结果 → stop_search 收口
- 已确认未找到匹配（候选为空或得分都很低）→ stop_search confidence ≤ 0.2 且在 reason 中明确说"未找到"
- 系统会在第 10 轮强制停止
- 如果你不调用任何工具也不调用 stop_search，系统会注入提示要求你重新调用 stop_search
