...
async function parseQueryIntent(
  ...
  userQuery: string
): Promise<{ searchText: string; startTime: string | null; endTime: string | null }> {
  // 获取当前日期，用于相对时间计算
  // 例如: "上周" = 当前日期 - 7 天
...
只返回 JSON，不要其他内容。`;
  ...
  try {
    ...
    // 记录解析结果，方便验证
    console.log(`[Search] 意图解析完成: text="${searchText}", time=${startTime} to ${endTime}`);
...
async function searchModels(
  ...
  // 记录搜索参数
  console.log(`[Search] 执行向量搜索: 阈值=${matchThreshold}, 数量=${matchCount}`);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    const matchThreshold = typeof threshold === 'number' ? Math.max(0, Math.min(1, threshold)) : 0.5;
    ...
    // 5a. 解析用户意图
    const { searchText, startTime, endTime } = await parseQueryIntent(aiClient, query);
    ...
    const results = await searchModels(supabase, queryVector, matchThreshold, 10, startTime, endTime);
