...
def normalize_lookup_terms(*terms: str) -> list[str]:
    ...
    for term in terms:
        ...
        # Strip generic suffixes like “模型/内容/记录”.
        stripped = base
        changed = True
        while changed and len(stripped) >= 2:
            ...
            for suffix in GENERIC_OBJECT_SUFFIXES:
                if stripped.endswith(suffix) and len(stripped) > len(suffix):
                    ...
                    add(stripped)
                    changed = True
        # Pull quoted or bracketed core terms back into the candidate list.
...
def merge_and_rank_rows(
    ...
    *,
    ...
    ranked = sorted(
        merged_by_id.values(),
        key=lambda row: (
            count_lookup_term_matches(row, lookup_terms),
...
def retrieve_real_chain_case(
    *,
    ...
    retrieval = {
        ...
        "fallback_after_rpc_error": bool(rpc_errors) and retrieval_route == "lexical_fallback",
        "evidence": evidence,
    }
    return {
        "question": question,
