...
class PipelineConfig:
    ...
    # [新增] SAM3D 相关配置
    # 使用环境变量，默认为空（后续代码会自动处理）
    sam3d_repo_path: Path = field(default_factory=lambda: Path(
        os.getenv("SAM3D_REPO_PATH", str(BASE_DIR / "src/libs/sam-3d-objects"))
    ))
    sam3d_checkpoint_dir: Path = field(default_factory=lambda: Path(
        os.getenv("SAM3D_CHECKPOINT_DIR", str(BASE_DIR.parent.parent / "models/sam3d/checkpoints"))
    ))
